...
template <class frametype> class FrameRecv : public Runnable {
protected:
...
public:
  ...
  void finalize() { std::cout << "final steps\n"; }
...
template <class frametype> class FrameMean : public Runnable {
protected:
...
public:
  BufferedMean(ichannel<frametype, 5> chan, ochannel<frametype, 5> chan_out)
      : frames_(chan), frames_out_(chan_out) {}
...
int main() {
  ...
  // nodes
  ReadNode<frame_ptr, convertArrayToMat, 5> r(localData, frame_chan,
                                              header_chan);
