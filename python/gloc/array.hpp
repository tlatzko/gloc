...
#pragma GCC diagnostic warning "-Wold-style-cast"
    ...
    struct Array {
        ...
        inline size_type shape(size_type dim) const {
            gassert(0 <= dim && dim < ndimm, dim << " " << ndimm);
    ...
    private:
        ...
        void initialise() {
            gassert(is_object_convertible_to_array<type>(this->object_.ptr()),
                    numpy::getTypeID(T()) << " " <<
                    numpy::getArrayType(this->object_) << " " <<
                    ndimm << " " <<
                    numpy::getArrayNDims(this->object_));
