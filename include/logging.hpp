...
/// @brief Static class that contains helper functions for el::Level
  ...
  public:
    ...
    static inline void
    forEachLevel (base::type::EnumType* startIndex, const std::function<bool
    (void)>& fn)
    {
...
/// @brief Static class that contains helper functions for el::ConfigurationType
  ...
  public:
    ...
    {
      if ((strcmp (configStr, "ENABLED") == 0)
	  || (strcmp (configStr, "enabled") == 0))
	return ConfigurationType::Enabled;
      if ((strcmp (configStr, "TO_FILE") == 0)
	  || (strcmp (configStr, "to_file") == 0))
	return ConfigurationType::ToFile;
      if ((strcmp (configStr, "TO_STANDARD_OUTPUT") == 0)
	  || (strcmp (configStr, "to_standard_output") == 0))
	return ConfigurationType::ToStandardOutput;
      if ((strcmp (configStr, "FORMAT") == 0)
	  || (strcmp (configStr, "format") == 0))
	return ConfigurationType::Format;
      if ((strcmp (configStr, "FILENAME") == 0)
	  || (strcmp (configStr, "filename") == 0))
	return ConfigurationType::Filename;
      if ((strcmp (configStr, "MILLISECONDS_WIDTH") == 0)
	  || (strcmp (configStr, "milliseconds_width") == 0))
	return ConfigurationType::MillisecondsWidth;
      if ((strcmp (configStr, "PERFORMANCE_TRACKING") == 0)
	  || (strcmp (configStr, "performance_tracking") == 0))
	return ConfigurationType::PerformanceTracking;
      if ((strcmp (configStr, "MAX_LOG_FILE_SIZE") == 0)
	  || (strcmp (configStr, "max_log_file_size") == 0))
	return ConfigurationType::MaxLogFileSize;
      if ((strcmp (configStr, "LOG_FLUSH_THRESHOLD") == 0)
	  || (strcmp (configStr, "log_flush_threshold") == 0))
    ...
    static inline void
    forEachConfigType (base::type::EnumType* startIndex,
...
/// @brief Namespace containing constants used internally.
    namespace consts
    {
      ...
      static const base::type::char_t* kInfoLevelLogValue = ELPP_LITERAL(
	  "INFO ");
      static const base::type::char_t* kDebugLevelLogValue = ELPP_LITERAL(
	  "DEBUG");
      static const base::type::char_t* kWarningLevelLogValue = ELPP_LITERAL(
	  "WARN ");
      static const base::type::char_t* kErrorLevelLogValue = ELPP_LITERAL(
	  "ERROR");
      static const base::type::char_t* kFatalLevelLogValue = ELPP_LITERAL(
	  "FATAL");
      static const base::type::char_t* kVerboseLevelLogValue = ELPP_LITERAL(
	  "VER");
      static const base::type::char_t* kTraceLevelLogValue = ELPP_LITERAL(
	  "TRACE");
      static const base::type::char_t* kInfoLevelShortLogValue = ELPP_LITERAL(
	  "I");
      static const base::type::char_t* kDebugLevelShortLogValue = ELPP_LITERAL(
	  "D");
      static const base::type::char_t* kWarningLevelShortLogValue =
	  ELPP_LITERAL("W");
      static const base::type::char_t* kErrorLevelShortLogValue = ELPP_LITERAL(
	  "E");
      static const base::type::char_t* kFatalLevelShortLogValue = ELPP_LITERAL(
	  "F");
      static const base::type::char_t* kVerboseLevelShortLogValue =
	  ELPP_LITERAL("V");
      static const base::type::char_t* kTraceLevelShortLogValue = ELPP_LITERAL(
	  "T");
      // Format specifiers - These are used to define log format
      static const base::type::char_t* kAppNameFormatSpecifier = ELPP_LITERAL(
	  "%app");
      static const base::type::char_t* kLoggerIdFormatSpecifier = ELPP_LITERAL(
	  "%logger");
      static const base::type::char_t* kThreadIdFormatSpecifier = ELPP_LITERAL(
	  "%thread");
      ...
      static const base::type::char_t* kDateTimeFormatSpecifier = ELPP_LITERAL(
	  "%datetime");
      static const base::type::char_t* kLogFileFormatSpecifier = ELPP_LITERAL(
	  "%file");
      static const base::type::char_t* kLogFileBaseFormatSpecifier =
	  ELPP_LITERAL("%fbase");
      static const base::type::char_t* kLogLineFormatSpecifier = ELPP_LITERAL(
	  "%line");
	  ...
	  ELPP_LITERAL("%host");
      static const base::type::char_t* kMessageFormatSpecifier = ELPP_LITERAL(
	  "%msg");
	...
	{ "January", "February", "March", "Apri", "May", "June", "July",
...
#   endif  // ELPP_OS_UNIX
      ...
      } kTimeFormats[] =
	{
      ...
      static const int kTimeFormatsCount = sizeof(kTimeFormats)
	...
	const char* detail;
      ...
      static const int kCrashSignalsCount = sizeof(kCrashSignals)
	  / sizeof(kCrashSignals[0]);
    }  // namespace consts
  }  // namespace base
  typedef std::function<void
  (const char*, std::size_t)> PreRollOutCallback;
  namespace base
  {
    static inline void
    defaultPreRollOutCallback (const char*, std::size_t)
    {
    }
...
/// @brief A milliseconds width class containing actual width and offset for date/time
	...
	init (width);
      ...
      bool
      operator== (const MillisecondsWidth& msWidth)
      {
...
/// @brief Base of thread safe class, this class is inheritable-only
	...
	static std::string
	extractPathFromFilename (const std::string& fullPath,
...
/// @brief String utilities helper class used internally. You should not use it.
	...
	endsWith (const std::string& str, const std::string& end)
	{
	  return (str.length () >= end.length ())
	      && (str.compare (str.length () - end.length (), end.length (),
			       end) == 0);
	}
	...
	static inline std::string&
	replaceAll (std::string& str, const std::string& replaceWhat, // NOLINT
	...
	static void
	replaceFirstWithEscape (base::type::string_t& str,
	    ...
	    const base::type::string_t& replaceWith)
	{
	  ...
	  while ((foundAt = str.find (replaceWhat, foundAt + 1))
	    ...
	    {
	      if (foundAt > 0
		  && str[foundAt - 1] == base::consts::kFormatSpecifierChar)
		{
	...
	static inline char*
	convertAndAddToBuff (std::size_t n, int len, char* buf,
...
/// @brief Operating System helper static class used internally. You should not use it.
	...
	currentUser (void)
	{
	  ...
	  return getEnvironmentVariable ("USER", base::consts::kUnknownUser,
					 "whoami");
#elif ELPP_OS_WINDOWS
	...
	termSupportsColor (void)
	{
	  ...
	  return term == "xterm" || term == "xterm-color"
	      || term == "xterm-256color" || term == "screen" || term == "linux"
	      || term == "cygwin";
	}
      };
...
#   endif  // ELPP_COMPILER_MSVC || defined(_MSC_EXTENSIONS)
	...
	static inline std::string
	getDateTime (const char* format, const base::MillisecondsWidth* msWidth)
	{
	  ...
	  parseFormat (
	      buff_, kBuffSize, format, &timeInfo,
	      static_cast<std::size_t> (currTime.tv_usec / msWidth->m_offset),
	      msWidth);
	...
	formatTime (unsigned long long time, base::TimestampUnit timestampUnit)
	{
	  ...
	  base::type::EnumType start =
	      static_cast<base::type::EnumType> (timestampUnit);
	  const base::type::char_t* unit =
	      base::consts::kTimeFormats[start].unit;
	  for (base::type::EnumType i = start;
	      i < base::consts::kTimeFormatsCount - 1; ++i)
	    {
			   ...
			   base::TimestampUnit timestampUnit)
	{
	  if (timestampUnit == base::TimestampUnit::Microsecond)
	    {
	      return static_cast<unsigned long long> (static_cast<unsigned long long> (1000000
	    ...
	    {
	      return static_cast<unsigned long long> ((((endTime.tv_sec
...
#   endif  // ELPP_COMPILER_MSVC
	...
	static char*
	parseFormat (char* buf, std::size_t bufSz, const char* format,
	      ...
	      if (*format == base::consts::kFormatSpecifierChar)
		{
		  switch (*++format)
		    ...
		    case 'd':  // Day
			  ...
			  tInfo->tm_mday, 2, buf, bufLim);
		      continue;
		    case 'a':  // Day of week (short)
		      buf = base::utils::Str::addToBuff (
			  base::consts::kDaysAbbrev[tInfo->tm_wday], buf,
			  bufLim);
		      continue;
		    case 'A':  // Day of week (long)
		      buf = base::utils::Str::addToBuff (
			  base::consts::kDays[tInfo->tm_wday], buf, bufLim);
		      continue;
		    case 'M':  // month
		      buf = base::utils::Str::convertAndAddToBuff (
			  tInfo->tm_mon + 1, 2, buf, bufLim);
		      continue;
		    case 'b':  // month (short)
		      buf = base::utils::Str::addToBuff (
			  base::consts::kMonthsAbbrev[tInfo->tm_mon], buf,
			  bufLim);
		      continue;
		    case 'B':  // month (long)
		      buf = base::utils::Str::addToBuff (
		    ...
		    case 'y':  // year (two digits)
		      buf = base::utils::Str::convertAndAddToBuff (
			  tInfo->tm_year + base::consts::kYearBase, 2, buf,
			  bufLim);
		      continue;
		    case 'Y':  // year (four digits)
		      buf = base::utils::Str::convertAndAddToBuff (
			  tInfo->tm_year + base::consts::kYearBase, 4, buf,
			  bufLim);
		      continue;
		    case 'h':  // hour (12-hour)
		      buf = base::utils::Str::convertAndAddToBuff (
			  tInfo->tm_hour % 12, 2, buf, bufLim);
		      continue;
		    case 'H':  // hour (24-hour)
		      buf = base::utils::Str::convertAndAddToBuff (
			  tInfo->tm_hour, 2, buf, bufLim);
		      continue;
		    case 'm':  // minute
		      buf = base::utils::Str::convertAndAddToBuff (
			  tInfo->tm_min, 2, buf, bufLim);
		      continue;
		    case 's':  // second
		      buf = base::utils::Str::convertAndAddToBuff (
			  tInfo->tm_sec, 2, buf, bufLim);
		      continue;
		    case 'z':  // milliseconds
		    case 'g':
		      buf = base::utils::Str::convertAndAddToBuff (
			  msec, msWidth->m_width, buf, bufLim);
		      continue;
		    case 'F':  // AM/PM
		      buf = base::utils::Str::addToBuff (
			  (tInfo->tm_hour >= 12) ?
			      base::consts::kPm : base::consts::kAm,
			  buf, bufLim);
		      continue;
		    default:
		      continue;
...
/// @brief Command line arguments for application if specified using el::Helpers::setArgs(..) or START_EASYLOGGINGPP(..)
	  ...
	  for (int i = 1; i < m_argc; ++i)
	      ...
	      if (v != nullptr && strlen (v) > 0)
		{
		  ...
		  if (hasParamWithValue (key.c_str ()))
		    ...
		    {
		      m_paramsWithValue.insert (
			  std::make_pair (key, std::string (v + 1)));
		    }
		}
	      if (v == nullptr)
		{
	...
	{
	  return std::find (m_params.begin (), m_params.end (),
			    std::string (paramKey)) != m_params.end ();
	}
	...
	inline friend base::type::ostream_t&
...
///         explicitly (by using lock functions)
	...
	private:
	  virtual inline void
	  deepCopy (const AbstractRegistry<T_Ptr, std::map<T_Key, T_Ptr*>>& sr) ELPP_FINAL
	  {
...
/// should be made thread-safe explicitly
	...
	class RegistryWithPred : public AbstractRegistry<T_Ptr,
	    std::vector<T_Ptr*>>
	{
	public:
	  ...
	  friend inline base::type::ostream_t&
	  operator<< (base::type::ostream_t& os, const RegistryWithPred& sr)
	  {
	    for (const_iterator it = sr.list ().begin ();
		it != sr.list ().end (); ++it)
	      {
	...
	protected:
	  ...
	  template<typename T, typename T2>
	    ...
	    {
	      iterator iter = std::find_if (this->list ().begin (),
					    this->list ().end (),
					    Pred (arg1, arg2));
...
/// @brief Represents log format containing flags and date format. This is used internally to start initial log
    ...
    public:
      LogFormat (void) :
	  m_level (Level::Unknown), m_userFormat (base::type::string_t ()), m_format (
	      base::type::string_t ()), m_dateTimeFormat (std::string ()), m_flags (
	      0x0)
      {
      }
	...
	if ((dateIndex = formatCopy.find (
	    base::consts::kDateTimeFormatSpecifier)) != std::string::npos)
	  {
	    while (dateIndex > 0
		&& formatCopy[dateIndex - 1]
		    == base::consts::kFormatSpecifierChar)
	      {
		dateIndex = formatCopy.find (
		    base::consts::kDateTimeFormatSpecifier, dateIndex + 1);
	      }
	...
	if ((currFormat.size () > index) && (ptr[0] == '{'))
	...
	else
	  {
		...
		m_dateTimeFormat = std::string (
		    base::consts::kDefaultDateTimeFormat);
	      }
	  }
      }
	...
	if (hasFlag (base::FormatFlags::User))
	  {
	    ...
	    base::utils::Str::replaceFirstWithEscape (
		m_format, base::consts::kCurrentUserFormatSpecifier,
		base::utils::s_currentUser);
	  }
	if (hasFlag (base::FormatFlags::Host))
	  {
	    base::utils::Str::replaceFirstWithEscape (
		m_format, base::consts::kCurrentHostFormatSpecifier,
    ...
    private:
      ...
      // To resolve loggerId format specifier easily
    };
  }  // namespace base
/// @brief Resolving function for format specifier
  typedef std::function<const char*
  (void)> FormatSpecifierValueResolver;
...
///   * el::Configuration confFilenameInfo(el::Level::Info, el::ConfigurationType::Filename, "/var/log/my.log");
  ...
  public:
	...
	m_level (c.m_level), m_configurationType (c.m_configurationType), m_value (
	    c.m_value)
    {
    }
	...
	m_level (level), m_configurationType (configurationType), m_value (
	    value)
    {
    }
...
/// @detail This repository represents configurations for all the levels and configuration type mapped to a value.
  class Configurations : public base::utils::RegistryWithPred<Configuration,
  ...
  public:
    ...
    Configurations (const std::string& configurationFile,
		    bool useDefaultsForRemaining = true, Configurations* base =
			nullptr) :
    ...
    inline bool
    parseFromFile (const std::string& configurationFile, Configurations* base =
		       nullptr)
    {
    ...
    ///         do not proceed without successful parse.
    inline bool
    parseFromText (const std::string& configurationsString,
    ...
    inline void
    set (Level level, ConfigurationType configurationType,
	 const std::string& value)
    {
      base::threading::ScopedLock scopedLock (lock ());
    ...
    setToDefault (void)
    {
      ...
      setGlobally (ConfigurationType::ToStandardOutput, std::string ("true"),
		   true);
      setGlobally (ConfigurationType::MillisecondsWidth, std::string ("3"),
		   true);
      ...
      setGlobally (ConfigurationType::LogFlushThreshold, std::string ("0"),
		   true);
    ...
    setRemainingToDefault (void)
    {
...
#endif  // !defined(ELPP_NO_DEFAULT_LOG_FILE)
      unsafeSetIfNotExist (Level::Global, ConfigurationType::ToFile,
			   std::string ("true"));
      unsafeSetIfNotExist (Level::Global, ConfigurationType::ToStandardOutput,
			   std::string ("true"));
      unsafeSetIfNotExist (Level::Global, ConfigurationType::MillisecondsWidth,
			   std::string ("3"));
      unsafeSetIfNotExist (Level::Global,
      ...
      unsafeSetIfNotExist (Level::Fatal, ConfigurationType::Format,
			   std::string ("%datetime %level [%logger] %msg"));
      unsafeSetIfNotExist (
	  Level::Verbose, ConfigurationType::Format,
	  std::string ("%datetime %level-%vlevel [%logger] %msg"));
      unsafeSetIfNotExist (
    ...
    public:
      ...
      ///         do not proceed without successful parse.
      static bool
      parseFromFile (const std::string& configurationFile,
		     Configurations* sender, Configurations* base = nullptr)
      {
	sender->setFromBase (base);
	std::ifstream fileStream_ (configurationFile.c_str (),
				   std::ifstream::in);
	ELPP_ASSERT(
	    fileStream_.is_open (),
	...
	while (fileStream_.good ())
	  {
	    ...
	    parsedSuccessfully = parseLine (&line, &currConfigStr,
					    &currLevelStr, &currLevel, sender);
	...
	if ((foundAt = line->find (base::consts::kConfigurationComment))
	    != std::string::npos)
	  {
	    if (foundAt < quotesEnd)
	      {
		foundAt = line->find (base::consts::kConfigurationComment,
				      quotesEnd + 1);
	      }
	...
	return line != ""
	    && (line[0] >= 65 || line[0] <= 90 || line[0] >= 97
		|| line[0] <= 122) && (assignment != std::string::npos)
	    && (line.size () > assignment);
      }

      static bool
      parseLine (std::string* line, std::string* currConfigStr,
	...
	if (isLevel (*line))
	  {
	    ...
	    *currLevel = LevelHelper::convertFromString (
		currLevelStr->c_str ());
	    return true;
	  }
	if (isConfig (*line))
	  {
	    ...
	    *currConfigStr = base::utils::Str::trim (*currConfigStr);
	    currConfig = ConfigurationTypeHelper::convertFromString (
		currConfigStr->c_str ());
		...
		if ((quotesStart != quotesEnd)
		    && (quotesStart + 1 != quotesEnd))
		  {
		    // Explicit check in case if assertion is disabled
		    currValue = currValue.substr (quotesStart + 1,
						  quotesEnd - 1);
		  }
	      }
	  }
  ...
  private:
    ...
    void
    unsafeSetIfNotExist (Level level, ConfigurationType configurationType,
			 const std::string& value)
    {
      Configuration* conf = RegistryWithPred<Configuration,
	  Configuration::Predicate>::get (level, configurationType);
      if (conf == nullptr)
	{
    ...
    void
    unsafeSet (Level level, ConfigurationType configurationType,
	       const std::string& value)
    {
      Configuration* conf = RegistryWithPred<Configuration,
	  Configuration::Predicate>::get (level, configurationType);
    ...
    /// @see Configurations::setGlobally(ConfigurationType configurationType, const std::string& value)
    void
    unsafeSetGlobally (ConfigurationType configurationType,
...
/// This is thread safe and final class containing non-virtual destructor (means nothing should inherit this class)
	...
	return getConfigByVal<bool> (level, &m_toStandardOutputMap,
      ...
      inline const base::LogFormat&
      logFormat (Level level)
      {
	return getConfigByRef<base::LogFormat> (level, &m_logFormatMap,
						"logFormat");
      }

      inline const base::MillisecondsWidth&
      millisecondsWidth (Level level = Level::Global)
      {
	return getConfigByRef<base::MillisecondsWidth> (level,
							&m_millisecondsWidthMap,
							"millisecondsWidth");
      }
	...
	inline Conf_T
	getConfigByVal (Level level, const std::map<Level, Conf_T>* confMap,
			const char* confName)
	{
	  base::threading::ScopedLock scopedLock (lock ());
      ...
      template<typename Conf_T>
	inline Conf_T&
	getConfigByRef (Level level, std::map<Level, Conf_T>* confMap,
			const char* confName)
	{
	  base::threading::ScopedLock scopedLock (lock ());
      ...
      template<typename Conf_T>
	inline Conf_T
	unsafeGetConfigByVal (Level level,
			      const std::map<Level, Conf_T>* confMap,
			      const char* confName)
	{
	  ELPP_UNUSED(confName);
	  typename std::map<Level, Conf_T>::const_iterator it = confMap->find (
	      level);
	  if (it == confMap->end ())
	    {
	      try
      ...
      template<typename Conf_T>
	inline Conf_T&
	unsafeGetConfigByRef (Level level, std::map<Level, Conf_T>* confMap,
			      const char* confName)
	{
	  ELPP_UNUSED(confName);
      ...
      template<typename Conf_T>
	void
	setValue (Level level, const Conf_T& value,
		  std::map<Level, Conf_T>* confMap, bool includeGlobalLevel =
		      true)
	{
	  ...
	  typename std::map<Level, Conf_T>::iterator it = confMap->find (
	      Level::Global);
	...
	for (Configurations::const_iterator it = configurations->begin ();
	    it != configurations->end (); ++it)
	  {
	    ...
	    if (conf->configurationType () == ConfigurationType::Enabled)
	      {
		setValue (conf->level (), getBool (conf->value ()),
			  &m_enabledMap);
	      }
		...
		setValue (
		    Level::Global,
		    base::MillisecondsWidth (
			static_cast<int> (getULong (conf->value ()))),
		    &m_millisecondsWidthMap);
	      }
	    else if (conf->configurationType ()
		== ConfigurationType::PerformanceTracking)
		...
		== ConfigurationType::MaxLogFileSize)
	      {
		setValue (conf->level (),
			  static_cast<std::size_t> (getULong (conf->value ())),
			  &m_maxLogFileSizeMap);
#if !defined(ELPP_NO_DEFAULT_LOG_FILE)
	...
	for (std::vector<Configuration*>::iterator conf =
	    withFileSizeLimit.begin (); conf != withFileSizeLimit.end ();
	    ++conf)
	  {
	...
	std::size_t dateIndex = std::string::npos;
	std::string dateTimeFormatSpecifierStr = std::string (
	    base::consts::kDateTimeFormatSpecifierForFilename);
	if ((dateIndex = resultingFilename.find (
	    dateTimeFormatSpecifierStr.c_str ())) != std::string::npos)
	  {
	    while (dateIndex > 0
		&& resultingFilename[dateIndex - 1]
		    == base::consts::kFormatSpecifierChar)
	      {
		dateIndex = resultingFilename.find (
		    dateTimeFormatSpecifierStr.c_str (), dateIndex + 1);
	      }
	    if (dateIndex != std::string::npos)
	      {
		...
		if ((resultingFilename.size () > dateIndex) && (ptr[0] == '{'))
		  {
		    ...
		    resultingFilename.erase (
			dateIndex + dateTimeFormatSpecifierStr.size (), count);
		    fmt = ss.str ();
		  }
		else
		  {
		    fmt = std::string (
			base::consts::kDefaultDateTimeFormatInFilename);
		  }
		base::MillisecondsWidth msWidth (3);
		std::string now = base::utils::DateTime::getDateTime (
		    fmt.c_str (), &msWidth);
	...
	create (
	    m_filenameMap.empty () && m_fileStreamMap.empty () ?
		Level::Global : level);
      }

      bool
      unsafeValidateFileRolling (Level level,
				 const PreRollOutCallback& PreRollOutCallback)
      {
	base::type::fstream_t* fs = unsafeGetConfigByRef (level,
							  &m_fileStreamMap,
							  "fileStream").get ();
	if (fs == nullptr)
	  {
	...
	std::size_t maxLogFileSize = unsafeGetConfigByVal (level,
							   &m_maxLogFileSizeMap,
							   "maxLogFileSize");
	std::size_t currFileSize = base::utils::File::getSizeOfFile (fs);
	if (maxLogFileSize != 0 && currFileSize >= maxLogFileSize)
	  {
	    std::string fname = unsafeGetConfigByRef (level, &m_filenameMap,
						      "filename");
...
/// @brief Class that keeps record of current line hit for occasional logging
    ...
    public:
      HitCounter (void) :
	  m_filename (""), m_lineNumber (0), m_hitCounts (0)
      {
      }
	  ...
	  m_filename (hitCounter.m_filename), m_lineNumber (
	      hitCounter.m_lineNumber), m_hitCounts (hitCounter.m_hitCounts)
      {
      }
	...
	return *this;
      ...
      inline void
      resetLocation (const char* filename, unsigned long int lineNumber)
      {
	m_filename = filename;
...
/// @brief Repository for hit counters used across the application
	...
	base::HitCounter, base::HitCounter::Predicate>
    {
    public:
      ...
      bool
      validateEveryN (const char* filename, unsigned long int lineNumber,
		      std::size_t n)
      {
	base::threading::ScopedLock scopedLock (lock ());
	...
	bool result = (n >= 1 && counter->hitCounts () != 0
	    && counter->hitCounts () % n == 0);
	return result;
      }
      ...
      /// @return True if validation resulted in triggering hit. Meaning logs should be written everytime true is returned
      bool
      validateAfterN (const char* filename, unsigned long int lineNumber,
		      std::size_t n)
      {
	base::threading::ScopedLock scopedLock (lock ());
	base::HitCounter* counter = get (filename, lineNumber);
	...
	return false;
      ...
      /// @return True if validation resulted in triggering hit. Meaning logs should be written everytime true is returned
      bool
      validateNTimes (const char* filename, unsigned long int lineNumber,
		      std::size_t n)
      {
	base::threading::ScopedLock scopedLock (lock ());
	base::HitCounter* counter = get (filename, lineNumber);
	...
	return false;
      ...
      getCounter (const char* filename, unsigned long int lineNumber)
      {
	base::threading::ScopedLock scopedLock (lock ());
...
{
  if (!base::utils::s_termSupportsColor)
    return;
...
public:
  Logger (const std::string& id,
	  base::LogStreamsReferenceMap* logStreamsReference) :
      m_id (id), m_typedConfigurations (nullptr), m_parentApplicationName (
	  std::string ()), m_isConfigured (false), m_logStreamsReference (
	  logStreamsReference)
  {
    initUnflushedCount ();
  }
  ...
  configure (const Configurations& configurations)
  {
    ...
    if (m_configurations != configurations)
      {
	m_configurations.setFromBase (
	    const_cast<Configurations*> (&configurations));
      }
    base::utils::safeDelete (m_typedConfigurations);
    m_typedConfigurations = new base::TypedConfigurations (
	&m_configurations, m_logStreamsReference);
  ...
  isValidId (const std::string& id)
  {
    for (std::string::const_iterator it = id.begin (); it != id.end (); ++it)
      {
	if (!base::utils::Str::contains (base::consts::kValidLoggerIdSymbols,
					 *it))
	  {
	    return false;
	  }
...
/// @brief Loggers repository
	...
	if (logger_ == nullptr && forceCreation)
	  {
		...
		ELPP_ASSERT(
		    validId,
		    "Invalid logger ID [" << id << "]. Not registering this logger.");
		return nullptr;
	      }
	    logger_ = new Logger (id, m_defaultConfigurations,
	...
	for (base::LogStreamsReferenceMap::iterator it =
	    m_logStreamsReference.begin (); it != m_logStreamsReference.end ();
	    ++it)
	  {
...
/// @brief Represents registries for verbose logging
	...
	base::threading::ScopedLock scopedLock (lock ());
	auto addSuffix =
	    [](std::stringstream& ss, const char* sfx, const char* prev)
	      {
	...
	auto insert =
	    [&](std::stringstream& ss, base::type::VerboseLevel level)
	      {
	...
	for (; *modules; ++modules)
	  {
		...
		if (isMod)
		...
		else if (isLevel)
		  {
		    if (isdigit (*modules))
		      {
			level = static_cast<base::type::VerboseLevel> (*modules)
			    - 48;
		      }
		  }
		break;
	...
	if (m_modules.empty () || file == nullptr)
	  ...
	  {
	    std::map<std::string, base::type::VerboseLevel>::iterator it =
		m_modules.begin ();
	    ...
	    if (base::utils::hasFlag (
		LoggingFlag::AllowVerboseIfModuleNotSpecified, *m_pFlags))
	      {
		return true;
	      }
	    return false;
      ...
      inline const std::map<std::string, base::type::VerboseLevel>&
      modules (void) const
      {
	return m_modules;
      }
  ...
  {
  public:
    LogMessage (Level level, const std::string& file, unsigned long int line,
		const std::string& func, base::type::VerboseLevel verboseLevel,
		Logger* logger) :
	m_level (level), m_file (file), m_line (line), m_func (func), m_verboseLevel (
	    verboseLevel), m_logger (logger), m_message (
...
/// @brief Easylogging++ management storage
    ...
    public:
      ...
      Storage (const LogBuilderPtr& defaultLogBuilder) :
		  ...
		  0x0), m_vRegistry (new base::VRegistry (0, &m_flags)),
#if ELPP_ASYNC_LOGGING
	...
	Logger* performanceLogger = m_registeredLoggers->get (
	    std::string (base::consts::kPerformanceLoggerId));
	...
	installPerformanceTrackingCallback<
	    base::DefaultPerformanceTrackingCallback> (
	...
	base::utils::safeDelete (m_vRegistry);
      ...
      inline bool
      validateEveryNCounter (const char* filename, unsigned long int lineNumber,
			     std::size_t occasion)
      {
	return hitCounters ()->validateEveryN (filename, lineNumber, occasion);
      ...
      inline bool
      validateAfterNCounter (const char* filename, unsigned long int lineNumber,
			     std::size_t n)
      { // NOLINT
	return hitCounters ()->validateAfterN (filename, lineNumber, n);
      ...
      inline bool
      validateNTimesCounter (const char* filename, unsigned long int lineNumber,
			     std::size_t n)
      { // NOLINT
	...
	m_customFormatSpecifiers.push_back (customFormatSpecifier);
      ...
      {
	base::threading::ScopedLock scopedLock (lock ());
	std::vector<CustomFormatSpecifier>::iterator it = std::find (
	    m_customFormatSpecifiers.begin (), m_customFormatSpecifiers.end (),
	    formatSpecifier);
	...
	{
	  return installCallback<T, base::type::LogDispatchCallbackPtr> (
	...
	{
	  uninstallCallback<T, base::type::LogDispatchCallbackPtr> (
	      id, &m_logDispatchCallbacks);
	}
      template<typename T>
	inline T*
	...
	{
	  return callback<T, base::type::LogDispatchCallbackPtr> (
	      id, &m_logDispatchCallbacks);
	}
	...
	{
	  return installCallback<T, base::type::PerformanceTrackingCallbackPtr> (
	...
	{
	  uninstallCallback<T, base::type::PerformanceTrackingCallbackPtr> (
	      id, &m_performanceTrackingCallbacks);
	}
	...
	{
	  return callback<T, base::type::PerformanceTrackingCallbackPtr> (
	      id, &m_performanceTrackingCallbacks);
	}
    private:
	...
	// default log file
#if !defined(ELPP_DISABLE_LOG_FILE_FROM_ARG)
	if (m_commandLineArgs.hasParamWithValue (
	...
	inline bool
	installCallback (const std::string& id,
			 std::map<std::string, TPtr>* mapT)
	{
      ...
      template<typename T, typename TPtr>
	inline void
	uninstallCallback (const std::string& id,
			   std::map<std::string, TPtr>* mapT)
	{
	  if (mapT->find (id) != mapT->end ())
	    {
      ...
      template<typename T, typename TPtr>
	inline T*
	callback (const std::string& id, std::map<std::string, TPtr>* mapT)
	{
...
#define ELPP el::base::elStorage
	...
	dispatch (
	    std::move (
		m_data->logMessage ()->logger ()->logBuilder ()->build (
		    m_data->logMessage (),
...
#   endif  // defined(ELPP_SYSLOG)
			...
			namespace base
			{
			  ...
			  public:
			    base::type::string_t
			    build (const LogMessage* logMessage,
				   bool appendNewLine) const
			    {
			      base::TypedConfigurations* tc =
				  logMessage->logger ()->typedConfigurations ();
			      const base::LogFormat* logFormat =
				  &tc->logFormat (logMessage->level ());
			      ...
			      if (logFormat->hasFlag (
				  base::FormatFlags::AppName))
				{
				  // App name
				  base::utils::Str::replaceFirstWithEscape (
				      logLine,
				      base::consts::kAppNameFormatSpecifier,
				  ...
				  base::FormatFlags::ThreadId))
				{
				  ...
				  base::utils::Str::replaceFirstWithEscape (
				      logLine,
				      base::consts::kThreadIdFormatSpecifier,
				      base::threading::getCurrentThreadId ());
				}
			      if (logFormat->hasFlag (
				  base::FormatFlags::DateTime))
				{
				  // DateTime
				  base::utils::Str::replaceFirstWithEscape (
				      logLine,
				  ...
				  base::FormatFlags::Function))
				{
				  ...
				  base::utils::Str::replaceFirstWithEscape (
				      logLine,
				      base::consts::kLogFunctionFormatSpecifier,
				      logMessage->func ());
				}
			      if (logFormat->hasFlag (base::FormatFlags::File))
				{
				  ...
				  char* buf = base::utils::Str::clearBuff (
				      buff,
				      base::consts::kSourceFilenameMaxLength);
				  base::utils::File::buildStrippedFilename (
				      logMessage->file ().c_str (), buff);
				  buf = base::utils::Str::addToBuff (buff, buf,
								     bufLim);
				  base::utils::Str::replaceFirstWithEscape (
				      logLine,
				      base::consts::kLogFileFormatSpecifier,
				      std::string (buff));
				}
			      if (logFormat->hasFlag (
				  base::FormatFlags::FileBase))
				{
				  ...
				  char* buf = base::utils::Str::clearBuff (
				      buff,
				      base::consts::kSourceFilenameMaxLength);
				  base::utils::File::buildBaseFilename (
				      logMessage->file (), buff);
				  buf = base::utils::Str::addToBuff (buff, buf,
								     bufLim);
				  base::utils::Str::replaceFirstWithEscape (
				      logLine,
				      base::consts::kLogFileBaseFormatSpecifier,
				      std::string (buff));
				}
			      if (logFormat->hasFlag (base::FormatFlags::Line))
				{
				  // Line
				  char* buf = base::utils::Str::clearBuff (
				      buff, base::consts::kSourceLineMaxLength);
				  buf = base::utils::Str::convertAndAddToBuff (
				      logMessage->line (),
				      base::consts::kSourceLineMaxLength, buf,
				      bufLim, false);
				  base::utils::Str::replaceFirstWithEscape (
				      logLine,
				      base::consts::kLogLineFormatSpecifier,
				      std::string (buff));
				}
			      if (logFormat->hasFlag (
				  base::FormatFlags::Location))
				{
				  ...
				  buf = base::utils::Str::addToBuff (buff, buf,
								     bufLim);
				  buf = base::utils::Str::addToBuff (":", buf,
								     bufLim);
				  buf = base::utils::Str::convertAndAddToBuff (
				      logMessage->line (),
				      base::consts::kSourceLineMaxLength, buf,
				  ...
				  base::FormatFlags::LogMessage))
				{
				  ...
				  base::utils::Str::replaceFirstWithEscape (
				      logLine,
				      base::consts::kMessageFormatSpecifier,
				      logMessage->message ());
				}
...
/// @brief Dispatches log messages
			  ...
			  public:
				...
				m_proceed (proceed), m_logMessage (
				    std::move (logMessage)), m_dispatchAction (
			      ...
			      if (!m_proceed)
				{
			      ...
			      base::TypedConfigurations* tc =
				  m_logMessage.logger ()->m_typedConfigurations;
			      if (ELPP->hasFlag(LoggingFlag::StrictLogFileSizeCheck))
				{
				  ...
				  base::type::LogDispatchCallbackPtr>& h : ELPP->m_logDispatchCallbacks)
				{
...
// Log message builder
			  ...
			  public:
			    MessageBuilder (void) :
				m_logger (nullptr), m_containerLogSeperator (
				    ELPP_LITERAL(""))
			    {
			    }
			    void
...
/// @brief Main entry point of each logging
			  ...
			  {
			  public:
			    Writer (Level level, const char* file,
				    unsigned long int line, const char* func,
				    base::DispatchAction dispatchAction =
					base::DispatchAction::NormalLog,
				    base::type::VerboseLevel verboseLevel = 0) :
				m_level (level), m_file (file), m_line (line), m_func (
				    func), m_verboseLevel (verboseLevel), m_proceed (
			...
			protected:
			  ...
			  public:
				...
				base::DispatchAction dispatchAction =
				    base::DispatchAction::NormalLog,
				base::type::VerboseLevel verboseLevel = 0) :
				base::Writer (level, file, line, func,
					      dispatchAction, verboseLevel)
			    {
			    }
...
// Logging from Logger class. Why this is here? Because we have Storage and Writer class available
			...
			template<typename T, typename ... Args>
			  void
			  Logger::log_ (Level level, int vlevel, const char* s,
				...
				if (*s == base::consts::kFormatSpecifierChar)
				  {
					...
					++s;
				      ...
				      {
					if (*(s + 1)
					    == base::consts::kFormatSpecifierCharValue)
					  {
					    ++s;
					    b << value;
  ...
  template<typename T, typename ... Args>
    void
    Logger::log (Level level, const char* s, const T& value,
		 const Args&... args)
    {
      base::threading::ScopedLock scopedLock (lock ());
...
#   if ELPP_VERBOSE_LOG
  template<typename T, typename ... Args>
    inline void
    Logger::verbose (int vlevel, const char* s, const T& value,
		     const Args&... args)
    {
      base::threading::ScopedLock scopedLock (lock ());
...
#   define ELPP_CURR_FILE_PERFORMANCE_LOGGER el::base::consts::kPerformanceLoggerId
  ...
  public:
	...
	m_performanceTracker (nullptr), m_dataType (dataType), m_file (""), m_line (
	    0), m_func ("")
    {
    }
    inline const std::string*
  ...
  private:
    ...
    const char* m_func;
    inline void
    init (base::PerformanceTracker* performanceTracker, bool firstCheckpoint =
	      false)
    {
...
///        either when goes outside the scope of when checkpoint() is called
    class PerformanceTracker : public base::threading::ThreadSafe,
	public Loggable
    {
    public:
      PerformanceTracker (
	  ...
	  const std::string& loggerId = std::string (
	...
	el::base::Writer (m_data->performanceTracker ()->level (),
			  m_data->file (), m_data->line (), m_data->func ()).construct (
	    1, m_data->loggerId ().c_str ()) << ss.str ();
      }
    private:
...
/// @brief Contains some internal debugging tools like crash handler and stack tracer
	...
	{
	public:
	  StackTraceEntry (std::size_t index, const char* loc,
			   const char* demang, const char* hex,
			   const char* addr)
	  {
	    m_index = index;
...
/// @brief Logs reason of crash from sig
      static void
      logCrashReason (int sig, bool stackTraceIfAvailable, Level level,
		      const char* logger)
      {
	std::stringstream ss;
	ss << "CRASH HANDLED; ";
...
/// @brief Handles unexpected crashes
	...
	setHandler (const Handler& cHandler)
	{
	  ...
	  for (; i < base::consts::kCrashSignalsCount; ++i)
	    {
	      m_handler = signal (base::consts::kCrashSignals[i].numb,
				  cHandler);
	    }
	}
...
/// @brief Initializes syslog with process ID, options and facility. calls closelog() on d'tor
  ...
  {
  public:
    SysLogInitializer (const char* processIdent, int options = 0, int facility =
			   0)
    {
#if defined(ELPP_SYSLOG)
...
namespace el
{
  namespace base
  {
    namespace utils
    {
      template<typename T>
	static T*
	checkNotNull (T* ptr, const char* name, const char* loggers, ...)
	{
	  CLOG_IF(ptr == nullptr, FATAL, loggers) << "Check failed: [" << name
	      << " != nullptr]";
	  return ptr;
	}
    }  // namespace utils
