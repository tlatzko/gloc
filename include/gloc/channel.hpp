...
// TODO: investigate and ideally also discuss other scheduling algorithms
	...
	void
	_pre_blocking_recv (std::unique_lock<std::mutex>& lock)
	{
	...
	void
	_post_blocking_recv (std::unique_lock<std::mutex>& lock)
	{
	...
	template<class U>
	  bool
	  try_send (std::unique_lock<std::mutex>&, U&&);
	...
	_channel () :
	    m_mutex (), m_send_begin_cv (), m_send_end_cv (), m_recv_cv (), m_queue (), m_is_send_done (
		true), m_is_try_send_done (true), m_is_recv_ready (false), m_is_try_send_ready (
		false), m_is_try_recv_ready (false)
	{
	}
...
// TODO: investigate and ideally discuss pseudo-random distribution
  ...
  private:
	...
	template<class U, class V = typename std::remove_reference<U>::type>
	  static bool
	  _run (ochannel<V, N>& c, U&& u, NullaryFunction f)
	  {
      ...
      public:
	bool
	operator() (ochannel<T, N>& c, const T& t, NullaryFunction f)
	{
	  return _run (c, t, f);
	}

	bool
	operator() (ochannel<T, N>& c, T&& t, NullaryFunction f)
	{
    ...
    template<class T, std::size_t N, class NullaryFunction>
      ...
      {
	bool
	operator() (ichannel<T, N>& c, T& t, NullaryFunction f)
	{
    ...
    template<class T, std::size_t N, class UnaryFunction>
      ...
      {
	bool
	operator() (ichannel<T, N>& c, UnaryFunction f)
	{
	  internal::_channel<T, N>& _c = *c.m_channel_ptr;
	  std::unique_lock < std::mutex > lock (_c.mutex (), std::defer_lock);
	  if (lock.try_lock ())
  ...
  public:
    select () :
	m_try_functions (), random_gen (std::time (nullptr))
    {
    }
	...
	class U = typename std::remove_reference<T>::type>
      select&
      send_only (ochannel<U, N> c, T&& t)
      {
	return send (c, std::forward < T > (t), []()
	  { /* skip */});
      }
	...
	class U = typename std::remove_reference<T>::type>
      select&
      send (channel<U, N> c, T&& t, NullaryFunction f)
      {
	...
	class U = typename std::remove_reference<T>::type>
      select&
      send (ochannel<U, N> c, T&& t, NullaryFunction f)
      {
	m_try_functions.push_back (
	    std::bind (try_send_nullary<U, N, NullaryFunction> (), c,
		       std::forward < T > (t), f));
	return *this;
      }
    ...
    template<class T, std::size_t N, class NullaryFunction>
      select&
      recv (channel<T, N> c, T& t, NullaryFunction f)
      {
    ...
    template<class T, std::size_t N, class NullaryFunction>
      select&
      recv (ichannel<T, N> c, T& t, NullaryFunction f)
      {
	m_try_functions.push_back (
	    std::bind (try_recv_nullary<T, N, NullaryFunction> (), c,
		       std::ref (t), f));
	return *this;
      }
    ...
    template<class T, std::size_t N, class UnaryFunction>
      select&
      recv (channel<T, N> c, UnaryFunction f)
      {
    ...
    template<class T, std::size_t N, class UnaryFunction>
      select&
      recv (ichannel<T, N> c, UnaryFunction f)
      {
	m_try_functions.push_back (
    ...
    try_once ()
    {
      const try_functions::size_type n = m_try_functions.size (), i =
	  random_gen ();
    ...
    template<class Rep, class Period>
      void
      wait (const std::chrono::duration<Rep, Period>& sleep)
      {
	const try_functions::size_type n = m_try_functions.size ();
	try_functions::size_type i = random_gen ();
	for (;;)
  ...
  template<class T, std::size_t N>
    bool
    channel<T, N>::operator== (const ichannel<T, N>& other) const noexcept
    {
      return m_channel_ptr == other.m_channel_ptr;
    }
  ...
  template<class T, std::size_t N>
    bool
    channel<T, N>::operator== (const ochannel<T, N>& other) const noexcept
    {
      return m_channel_ptr == other.m_channel_ptr;
    }
  ...
  template<class T, std::size_t N>
    template<class U>
      bool
      internal::_channel<T, N>::try_send (std::unique_lock<std::mutex>& lock,
					  U&& u)
      {
	...
	m_queue.emplace_back (std::this_thread::get_id (),
			      std::forward < U > (u));
  ...
  template<class T, std::size_t N>
    template<class U>
      void
      internal::_channel<T, N>::_send (U&& u)
      {
	// unlock before notifying threads; otherwise, the
	// notified thread would unnecessarily block again
	  {
  ...
  template<class T, std::size_t N>
    void
    internal::_channel<T, N>::recv (T& t)
    {
      std::unique_lock < std::mutex > lock (m_mutex);
      _pre_blocking_recv (lock);
  ...
  template<class T, std::size_t N>
    std::unique_ptr<T>
    internal::_channel<T, N>::recv_ptr ()
    {
      std::unique_lock < std::mutex > lock (m_mutex);
      _pre_blocking_recv (lock);
