...
template <class T> struct _is_exception_safe {
  enum {
    value = true
  };
};
...
/**
 * FIXME
 * workaround for optional types
 * remove or true afer fix it
 */
template <class T>
...
template <class T, std::size_t N> class _channel {
  static_assert(
...
private:
  ...
  bool is_full() const { return m_queue.size() > N; }
  ...
  void _pre_blocking_recv(std::unique_lock<std::mutex> &lock) {
    ...
    m_recv_cv.wait(lock, [this] { return !this->m_queue.empty(); });
  ...
  template <class U> void _send(U &&);

public:
  // \pre: calling thread must own mutex()
  // \post: calling thread doesn't own mutex() anymore
  template <class U> bool try_send(std::unique_lock<std::mutex> &, U &&);

  // \pre: calling thread must own mutex()
  // \post: calling thread doesn't own mutex() anymore
  std::pair<bool, std::unique_ptr<T> >
  try_recv_ptr(std::unique_lock<std::mutex> &);

  _channel(const _channel &) = delete;

  // Propagates exceptions thrown by std::condition_variable constructor
  _channel()
      : m_mutex(), m_send_begin_cv(), m_send_end_cv(), m_recv_cv(), m_queue(),
        m_is_send_done(true), m_is_try_send_done(true), m_is_recv_ready(false),
        m_is_try_send_ready(false), m_is_try_recv_ready(false) {}

  // channel lock
  std::mutex &mutex() { return m_mutex; }

  // Propagates exceptions thrown by std::condition_variable::wait()
  void send(const T &t) { _send(t); }

  // Propagates exceptions thrown by std::condition_variable::wait()
  void send(T &&t) { _send(std::move(t)); }
...
template <class T, std::size_t N = 0> class channel {
  static_assert(
      N < std::numeric_limits<std::size_t>::max(),
      "N must be strictly less than the largest possible size_t value");
...
public:
  channel(const channel &other) noexcept : m_channel_ptr(other.m_channel_ptr) {}

  // Propagates exceptions thrown by std::condition_variable constructor
  channel() : m_channel_ptr(std::make_shared<internal::_channel<T, N> >()) {}
  ...
  // Propagates exceptions thrown by std::condition_variable::wait()
  void send(const T &t) { m_channel_ptr->send(t); }

  // Propagates exceptions thrown by std::condition_variable::wait()
  void send(T &&t) { m_channel_ptr->send(std::move(t)); }

  // Propagates exceptions thrown by std::condition_variable::wait()
  T recv() {
    static_assert(
        internal::_is_exception_safe<T>::value,
  ...
  // Propagates exceptions thrown by std::condition_variable::wait()
  std::unique_ptr<T> recv_ptr() { return m_channel_ptr->recv_ptr(); }

  // Propagates exceptions thrown by std::condition_variable::wait()
  void recv(T &t) { m_channel_ptr->recv(t); }
};

class select;

/// Can only be used to receive elements of type T
template <class T, std::size_t N = 0> class ichannel {
private:
  friend class select;
...
template <class T, std::size_t N = 0> class ochannel {
private:
  friend class select;
  friend class channel<T, N>;
  std::shared_ptr<internal::_channel<T, N> > m_channel_ptr;
...
private:
  ...
  template <class T, std::size_t N, class UnaryFunction> struct try_recv_unary {
...
public:
  select() : m_try_functions(), random_gen(std::time(nullptr)) {}
  ...
  template <class T, std::size_t N> select &recv_only(channel<T, N> c, T &t) {
    return recv_only(ichannel<T, N>(c), t);
  }

  template <class T, std::size_t N> select &recv_only(ichannel<T, N> c, T &t) {
  ...
  select &recv(ichannel<T, N> c, UnaryFunction f) {
    m_try_functions.push_back(
        std::bind(try_recv_unary<T, N, UnaryFunction>(), c, f));
    return *this;
  }
...
template <class T, std::size_t N>
template <class U>
bool internal::_channel<T, N>::try_send(std::unique_lock<std::mutex> &lock,
                                        U &&u) {
...
template <class T, std::size_t N>
std::pair<bool, std::unique_ptr<T> >
internal::_channel<T, N>::try_recv_ptr(std::unique_lock<std::mutex> &lock) {
...
void internal::_channel<T, N>::_send(U &&u) {
  ...
  // notified thread would unnecessarily block again
  {
    ...
    m_send_begin_cv.wait(lock, [this] {
      return this->m_is_send_done && this->m_is_try_send_done &&
             !this->is_full();
    });
  ...
  // wait (if necessary) until u has been received by another thread
  {
    ...
    m_send_end_cv.wait(lock, [this] { return !this->is_full(); });
    m_is_send_done = true;
  }
...
template <class T, std::size_t N> T internal::_channel<T, N>::recv() {
...
template <class T, std::size_t N> void internal::_channel<T, N>::recv(T &t) {
  std::unique_lock<std::mutex> lock(m_mutex);
  _pre_blocking_recv(lock);
