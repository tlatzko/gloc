...
protected:
    ...
    void init(){
        ...
        data_count_ = (static_cast<size_t>(end) - static_cast<size_t>(current_pos) / static_cast<size_t>(sizeof(LaserxaData)));
        
    }
    
public:
    
    LaserxaFile(const std::string& filename): filename_(filename){
        init();
    }
    ...
    const LaserxaInfo& get_info() const {
        return info_;
    }

    LaserxaData operator[](size_t i) {
...
//*************** general lxafunctions *************************
     ...
     * 
     */
    int get_direction_from_lxa(LaserxaFile& lxa){
 ...
 */
double time_delay_from_tint(double t_int, double t_frame=159.198, double t_reset=24.3, double t_lmi=4.1){
 ...
 */
size_t laser_offset_from_tint(double t_int, double t_frame=159.198, double t_reset=24.3, double t_lmi=4.1){
const double t_delay = time_delay_from_tint(t_int, t_frame, t_reset, t_lmi);
