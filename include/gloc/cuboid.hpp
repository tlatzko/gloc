...
public:
	...
	~CuboidFrame() {
        ...
        void setFrameHeader(){
            ...
            //            }
            std::memcpy((char*)&header, (char*)&pixel_values[0], sizeof(CuboidFrameHeader));
        }

        CuboidFrameHeader getFrameHeader(){
            return this->header;
        }
        ...
        std::string to_string(){
            ...
            ss << "fnumber: "<< header.frame_number
                ...
                << "\ntimestamp: "<< header.timestamp_ms << "ms\n";
            return ss.str();
        }
...
protected:
	...
	void readHeader() {
		...
		frame_count_ = (static_cast<long>(end) - 
                        static_cast<long>(first_frame_)) / 
...
public:
	 ...
	 * @param filename
	 */
	void loadCub(std::string filename) {
		filename_ = filename;
		if (file_in_.is_open()) {
			file_in_.close();
		}
